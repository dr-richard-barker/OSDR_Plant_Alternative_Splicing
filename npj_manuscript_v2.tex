...
\subsection{Cross-Study GO Enrichment Comparison Reveals a Core Splicing-Associated Signature}
...
A GO term $\times$ study heatmap (Figure~8) of the top 50 terms revealed both shared and stressor-specific enrichment patterns. The upset plot (Figure~9) quantified the overlap structure: 157 GO terms were significant in $\geq$2 studies, 31 in $\geq$3 studies, and 9 in $\geq$4 studies. The single most recurrent term was \textbf{nuclear speck (GO:0016607)}, significant in 6 of 7 studies (all except OSD-658), reflecting the universal involvement of spliceosomal machinery in the splicing response itself. Five terms were significant in $\geq$5 studies: nuclear speck, nucleic acid binding (GO:0003676, 5 studies), and protein binding (GO:0005515, 5 studies).

The 9 core terms significant in $\geq$4 studies (Table~\ref{tab:core_go}) were subjected to GO semantic similarity analysis using the Wang method with Arabidopsis annotation (Figure~10). The dendrogram revealed three functional clusters: (1) \textbf{RNA processing}---nuclear speck (GO:0016607), nucleus (GO:0005634), and RNA splicing (GO:0008380), reflecting the spliceosomal core; (2) \textbf{binding/catalytic}---nucleotide binding (GO:0000166), nucleic acid binding (GO:0003676), and protein binding (GO:0005515); and (3) \textbf{stress physiology}---circadian rhythm (GO:0007623), Mo-molybdopterin cofactor biosynthesis (GO:0006777), and abscisic acid biosynthesis (GO:0009688). The clustering of circadian rhythm with hormone biosynthesis rather than with RNA processing suggests that the shared splicing response targets regulatory physiology rather than merely the splicing machinery itself.

\begin{table}[htbp]
\centering
\caption{Core GO terms significant in $\geq$4 of 7 studies.}
\label{tab:core_go}
\begin{tabular}{lllll}
\hline
GO ID & Term & Ontology & n\_studies & Studies \\
\hline
GO:0016607 & nuclear speck & CC & 6 & 314, 120, 37, 678, 476, 251 \\
GO:0003676 & nucleic acid binding & MF & 5 & 314, 37, 678, 476, 251 \\
GO:0005515 & protein binding & MF & 5 & 314, 37, 678, 476, 251 \\
GO:0000166 & nucleotide binding & MF & 4 & 314, 678, 476, 251 \\
GO:0005634 & nucleus & CC & 4 & 314, 678, 476, 251 \\
GO:0006777 & Mo-molybdopterin biosynthesis & BP & 4 & 314, 678, 476, 251 \\
GO:0007623 & circadian rhythm & BP & 4 & 314, 120, 678, 476 \\
GO:0008380 & RNA splicing & BP & 4 & 314, 678, 476, 251 \\
GO:0009688 & abscisic acid biosynthesis & BP & 4 & 314, 678, 476, 251 \\
\hline
\end{tabular}
\end{table}

A stressor-grouped dot plot (Figure~11) of the top 5 terms per study (by fold enrichment) highlighted the stressor-specific signatures: ISS flight studies enriched for circadian rhythm and photoperiodism; OSD-658 (radiation) for phospholipid metabolism and translational stress; OSD-476 (regolith) for blue light photoreceptor activity and photosynthesis regulation; and OSD-251 (fractional gravity) for superoxide response (FDR=0.001, FE=488$\times$, the strongest single GO term across all studies). The largest pairwise GO overlaps were OSD-314 $\cap$ OSD-476 (32 terms), OSD-314 $\cap$ OSD-678 (28 terms), and OSD-476 $\cap$ OSD-678 (25 terms), consistent with the gene-level recurrence pattern.

\subsection{Batch Correction Improves Cross-Study ML Generalization}

To directly address the study-specific batch effects identified in the UMAP analysis, we applied two batch correction methods to the 199 $\times$ 9,875 PSI feature matrix: ComBat (parametric empirical Bayes, via sva) and limma::removeBatchEffect (non-parametric linear model). Both methods used study identity as the batch variable and preserved the treatment (is\_flight) covariate via a design matrix, removing study-specific variation while retaining the treatment signal. Corrected values were clipped to the [0, 1] PSI range. ComBat identified 5,580 genes with uniform expression within a single batch (all zeros) that were not adjusted.

The three matrices (uncorrected, ComBat, limma) were evaluated using the full ML pipeline (LOSOCV, nested 5-fold CV, UMAP, SHAP). The UMAP analysis provided the most reliable assessment of batch structure removal (Table~\ref{tab:batch_ml}, Figure~12): limma::removeBatchEffect nearly eliminated study-specific clustering (ARI(study) = 0.021, down from 0.995), while ComBat substantially reduced it (ARI(study) = 0.524). Both methods marginally increased the treatment clustering signal (ARI(treatment): uncorrected = $-$0.002, ComBat = 0.003, limma = 0.004), though the treatment signal remained negligible.

\begin{table}[htbp]
\centering
\caption{Batch correction ML comparison (3 matrices).}
\label{tab:batch_ml}
\begin{tabular}{lcccccc}
\hline
Matrix & LOSOCV RF & LOSOCV GBM & LOSOCV XGB & Nested k-fold & ARI(study) & ARI(treat) \\
\hline
Uncorrected & 0.475$\pm$0.082 & 0.507$\pm$0.150 & 0.491$\pm$0.172 & 0.613$\pm$0.037 & 0.995 & $-$0.002 \\
ComBat & 0.798$\pm$0.256 & 0.588$\pm$0.317 & 0.705$\pm$0.328 & 1.000* & 0.524 & 0.003 \\
limma & 0.553$\pm$0.056 & 0.581$\pm$0.152 & 0.614$\pm$0.127 & 0.656$\pm$0.053 & 0.021 & 0.004 \\
\hline
\end{tabular}
\\[2pt]
\footnotesize{*ComBat nested k-fold AUC = 1.0 is inflated by data leakage: batch correction was applied to the full matrix before CV splitting. LOSOCV and UMAP ARI are more reliable.}
\end{table}

LOSOCV performance improved with both correction methods (Figure~13): RF mean AUC increased from 0.475 (uncorrected) to 0.798 (ComBat) and 0.553 (limma). However, ComBat's improvement showed high variance ($\pm$0.256), with AUC=1.0 for OSD-251, OSD-476, and OSD-678 but AUC=0.39--0.49 for OSD-314 and OSD-37, indicating that ComBat over-corrects for some studies while under-correcting for others. limma produced more stable but modest improvements ($\pm$0.056). The nested k-fold AUC for limma (0.656 $\pm$ 0.053) represents a modest but genuine improvement over the uncorrected baseline (0.613 $\pm$ 0.037), while the ComBat nested k-fold AUC of 1.0 reflects data leakage from applying batch correction before cross-validation splitting.
